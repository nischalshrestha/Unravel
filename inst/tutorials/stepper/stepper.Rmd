...
stepper(
  ...
  explanations = list(
    "Testing: Here is a function `r kableExtra::text_spec(codeify('rename'), link='', popover = 'Alter axes labels. Function / dict values must be unique (1-to-1). Labels not contained in a dict / Series will be left as-is. Extra labels listed don’t throw an error. See the user guide for more.')` with a popover (hover over). Here is a `r color_text('highlighted element', 'highlighted')`. Here is a `r color_text('green element', 'inserted')`. Here is a `r color_text('red element', 'deleted')`. Here is a `r color_text('purple element', 'modified')`. Here are a bunch of words I want to highlight purple: `r color_text(c('date', 'start', 'box', 'away_team', 'away_points', 'home_team', 'home_points', 'n_ot'), 'modified')`.",
