...
stepper(
  ...
  explanations = list(
    "Testing: Here is a `r DataTutor::color_text('highlighted element', 'highlighted')`. Here is a `r DataTutor::color_text('green element', 'inserted')`. Here is a `r DataTutor::color_text('red element', 'deleted')`. Here is a `r DataTutor::color_text('purple element', 'modified')`. Here are a bunch of words I want to highlight purple: `r DataTutor::color_text(c('date', 'start', 'box', 'away_team', 'away_points', 'home_team', 'home_points', 'n_ot'), 'modified')`."
  ,
 "First, we renamed the old columns to `r DataTutor::color_text(c('date', 'start', 'box', 'away_team', 'away_points', 'home_team', 'home_points', 'n_ot'), 'modified')` using [`rename`](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.rename.html?highlight=rename#pandas.DataFrame.rename) and supplying our earlier `r DataTutor::color_text('column_names', 'highlighted')` dict. Now we have much more readable names. Note how we use a `.` to call the `rename` on the dataframe `nba`. Python is different from R in that it heavily uses the object-oriented style of programming using dot notations to access methods of classes (much like `R6` classes in R). This will take some getting used to since you are probably more familiar with R's functional style.",
