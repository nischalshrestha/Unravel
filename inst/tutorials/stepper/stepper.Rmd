...
<!-- nba = (nba.rename(columns=column_names).dropna(thresh=4)[['date', 'away_team', 'away_points', 'home_team', 'home_points']].assign(date=lambda x: pd.to_datetime(x['date'], format='%a, %b %d, %Y')).set_index('date', append=True).rename_axis(["game_id", "date"]).sort_index() -->
<!-- ) -->
<!-- nba -->
...
df_reactable <- function() {
  out <- NULL
  if (current == lastLine) {
    out <- python_df(df_outputs[[current - 1]])
  } else {
    out <- python_df(df_outputs[[current]])
  }
  reactable::reactable(
    out, 
    compact = TRUE,
